...
public class Screen implements java.io.Serializable{
	...
	public Player playerThree= new Player();
	public Player playerFour= new Player();
	public Player currentPlayer;
	public Dragon dragon = new Dragon();
	public boolean dragonLives = true;
	public RpiRunner lights = new RpiRunner();
	...
	private void initialize() {
		...
		btnLoadlocal = new JButton("Load Local");
				...
				play("/resources/blip.wav");
			      try {
			        ...
			        default: 
			         ...
			         //set lights
			         lights.reset();
			         lights.initialize(numPlayers);
			         lights.move(0, dragon.getSpace().getId());
			 		 lights.move(1, playerOne.getSpace().getId());
			         ...
			         i.printStackTrace();
			         btnLoadlocal.setText("Failed");
			      }catch(ClassNotFoundException c) {
			         System.out.println("Board class not found");
			         btnLoadlocal.setText("Error");
			         c.printStackTrace();
				}
			}
		});
		btnLoadlocal.setBackground(Color.WHITE);
		btnLoadlocal.setFont(new Font("Cambria", Font.BOLD, 16));
		btnLoadlocal.setBounds(175, 11, 139, 25);
		startPanel.add(btnLoadlocal);
		
		btnLoadUsb = new JButton("Load USB");
			...
			public void actionPerformed(ActionEvent e) {
			    ...
			    try {
			        ...
			        default: 
			         ...
			         i.printStackTrace();
			         btnLoadUsb.setText("Failed");
			    }catch(ClassNotFoundException c) {
			         System.out.println("Board class not found");
			         btnLoadUsb.setText("Error");
			         c.printStackTrace();
				}
			}
		});
		btnLoadUsb.setFont(new Font("Cambria", Font.BOLD, 16));
		btnLoadUsb.setBackground(Color.WHITE);
		btnLoadUsb.setBounds(10, 11, 139, 25);
		startPanel.add(btnLoadUsb);
		...
		btnSettings.setBounds(180, 11, 134, 29);
...
//		combatPanel.add(lblFighting2);
		...
		btnSaveLocal = new JButton("Save Locally");
		btnSaveLocal.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		         ...
		         try{
			         ...
			         out.close();
			         fileOut.close();
			         btnSaveLocal.setText("Saved");
			         
		    	  } catch (IOException i2){
		    		  i2.printStackTrace();
				      btnSaveLocal.setText("Error");
		    	  }
			}
		});
		btnSaveLocal.setFont(new Font("Dialog", Font.BOLD, 20));
		btnSaveLocal.setBackground(Color.WHITE);
		btnSaveLocal.setBounds(60, 116, 204, 66);
		settingsPanel.add(btnSaveLocal);
		...
		btnQuit.setBounds(60, 284, 204, 66);
		...
		btnSaveUSB = new JButton("Save Remotely");
		btnSaveUSB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				gameBoard.currentPlayer = currentPlayer;
		         gameBoard.dragonLives = dragonLives;
			     try {
			         ...
			         out.close();
			         fileOut.close();
			         btnSaveUSB.setText("Saved");
			         
			         System.out.println("Serialized data is saved in USB");
			      }catch(IOException  i) {
			    	  i.printStackTrace();
			    	  btnSaveUSB.setText("Failed");
			      }
			}
		});
		btnSaveUSB.setFont(new Font("Dialog", Font.BOLD, 20));
		btnSaveUSB.setBackground(Color.WHITE);
		btnSaveUSB.setBounds(60, 200, 204, 66);
		settingsPanel.add(btnSaveUSB);
	...
	public void repopulate(){
		...
		btnLoadUsb.setText("Load USB");
		btnLoadlocal.setText("Load Local");
		btnSaveLocal.setText("Save Locally");
	...
	public void spaceHandler(Space b,Space s){
		...
		if(s.getType()=="Castle Gates"){
			if(k[0]&&k[1]&&k[2]){
				...
				lblTurnOver.setText("NO TURNING BACK");
				rotatePanel.setVisible(true);
				return;
			} else {
		     ...
		     public void actionPerformed(ActionEvent e){
		         ...
		         if(playerOne.getSpace().getId() == dragon.getSpace().getId()){
		        	 ...
		        	 if(playerOne.hasMagicItem()&&playerOne.getMagicItem().getType()=="Mangel-Slayer"){
			        	 ...
			        	 playerOne.death();
			        	 lights.move(playerOne.getId(), playerOne.getSpace().getId());
		         ...
		         if(numPlayers > 1 && dragonLives && playerTwo.getSpace().getId() == dragon.getSpace().getId()){
		        	 ...
		        	 if(playerTwo.hasMagicItem()&&playerTwo.getMagicItem().getType()=="Mangel-Slayer"){
			        	 ...
			        	 playerTwo.death();
			        	 lights.move(playerTwo.getId(), playerTwo.getSpace().getId());
		         ...
		         if(numPlayers > 2 &&dragonLives && playerThree.getSpace().getId() == dragon.getSpace().getId()){
		        	 ...
		        	 if(playerThree.hasMagicItem()&&playerThree.getMagicItem().getType()=="Mangel-Slayer"){
			        	 ...
			        	 playerThree.death();
			        	 lights.move(playerThree.getId(), playerThree.getSpace().getId());
		         ...
		         if(numPlayers > 3 &&dragonLives && playerFour.getSpace().getId() == dragon.getSpace().getId()){
		        	 ...
		        	 if(playerFour.hasMagicItem()&&playerFour.getMagicItem().getType()=="Mangel-Slayer"){
			        	 ...
			        	 playerFour.death();
			        	 lights.move(playerFour.getId(), playerFour.getSpace().getId());
